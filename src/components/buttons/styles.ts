...
type ColorPalette = { main?: string; light?: string; text?: string };
...
function getPalette(
  ...
  return theme.colors?.[color as keyof NonNullable<ButtonTheme["colors"]>] as
    | ColorPalette
    | undefined;
}
  ...
  ({ color, fullWidth, size, theme }) => {
    ...
    return {
      ...
      "&:disabled": {
        ...
        backgroundColor:
          theme.colors?.text?.disabled ?? FALLBACK_COLORS.disabled,
