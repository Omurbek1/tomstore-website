...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = getNationalCityPage("bishkek")!;
  const url = `${SITE_URL}/${locale}/bishkek`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        ky: `${SITE_URL}/ky/bishkek`,
        "x-default": `${SITE_URL}/ru/bishkek`,
      },
    },
...
const CategoryCard = styled.div`
  ...
  &:hover { box-shadow: 0 4px 16px rgba(21,101,192,0.12); transform: translateY(-2px); }
  .icon { font-size: 28px; margin-bottom: 0.5rem; }
  .name { font-size: 13px; font-weight: 600; color: #1565c0; }
`;
...
export default async function BishkekPage({ params }: Props) {
  const { locale } = await params;
  const page = getNationalCityPage("bishkek")!;
  ...
  const pageSchema = {
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 3, name: isEn ? "Bishkek" : "Бишкек" },
      ],
    },
  };
    ...
    <AppLayout>
      ...
      <Box pt="20px" pb="60px">
        ...
        <CityHero>
          ...
          <p>{lead}</p>
          <BadgeRow>
            <Badge>🏙️ {isEn ? "Capital of Kyrgyzstan" : "Столица Кыргызстана"}</Badge>
            <Badge>👥 {isEn ? "1,100,000 residents" : "1 100 000 жителей"}</Badge>
            <Badge>🚚 {isEn ? "Delivery: 1 day" : "Доставка: 1 день"}</Badge>
            <Badge>✅ {isEn ? "Official warranty" : "Гарантия"}</Badge>
            <Badge>💳 {isEn ? "Installment 3–12 months" : "Рассрочка 3–12 мес."}</Badge>
            <Badge>📍 {isEn ? "Kalyk Akiev 66, Vesna Mall, 3rd floor, C47" : "ул. Калык Акиева 66, ТЦ Весна, 3-й этаж, С47"}</Badge>
          </BadgeRow>
        </CityHero>

        {/* Categories */}
        <H2 fontSize="20px" mb="1.25rem">
          {isEn ? "Electronics Categories in Bishkek" : "Категории электроники в Бишкеке"}
        </H2>
          ...
          {GEO_CATEGORIES.map((cat) => {
            ...
            return (
              <Grid item lg={2} sm={4} xs={6} key={cat.slug}>
                <CategoryCard as={Link} href={`/${locale}/${cat.slug}/bishkek`}>
        ...
        <H3 fontSize="15px" mb="0.75rem" color="text.muted">
          {isEn ? "Explore by category" : "Смотреть по категориям"}
        </H3>
