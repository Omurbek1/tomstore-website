...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = getNationalCategoryPage("noutbuki-kyrgyzstan")!;
  const url = `${SITE_URL}/${locale}/noutbuki-kyrgyzstan`;
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_URL}/ru/noutbuki-kyrgyzstan`,
      languages: {
        ru: `${SITE_URL}/ru/noutbuki-kyrgyzstan`,
        en: `${SITE_URL}/en/noutbuki-kyrgyzstan`,
        ky: `${SITE_URL}/ky/noutbuki-kyrgyzstan`,
        "x-default": `${SITE_URL}/ru/noutbuki-kyrgyzstan`,
      },
    },
...
export default async function NoutbukiKyrgyzstanPage({ params }: Props) {
  const { locale } = await params;
  const page = getNationalCategoryPage("noutbuki-kyrgyzstan")!;
  ...
  const pageSchema = {
    ...
    "@type": "WebPage",
    name: h1,
    description: seoText.slice(0, 200),
    url: `${SITE_URL}/${locale}/noutbuki-kyrgyzstan`,
    breadcrumb: {
      "@type": "BreadcrumbList",
      itemListElement: [
        ...
        { "@type": "ListItem", position: 3, name: isEn ? page.nameEn : page.nameRu },
      ],
    },
  };
    ...
    <AppLayout>
      <Box pt="20px" pb="60px">
        ...
        <NatHero>
          ...
          <BadgeRow>
            <Badge>💻 {isEn ? "200+ laptop models" : "200+ моделей ноутбуков"}</Badge>
            ...
            <Badge>🏷️ HP · ASUS · Acer · Lenovo · MSI</Badge>
          </BadgeRow>
        </NatHero>
        ...
        {products.length > 0 && (
          <Box mb="2.5rem">
            <H2 fontSize="18px" mb="1rem">
              {isEn ? "Laptops Available Now" : "Ноутбуки в наличии"}
            </H2>
            <ProductGrid>
            ...
            <Box textAlign="center">
              ...
              >
                {isEn ? "View all laptops →" : "Смотреть все ноутбуки →"}
              </Link>
            </Box>
          </Box>
        )}
        ...
        {/* Cities */}
        <H2 fontSize="18px" mb="1rem">
          {isEn ? "Laptops by City" : "Ноутбуки по городам"}
        </H2>
        <Box mb="2.5rem" display="flex" flexWrap="wrap" style={{ gap: "0.5rem" }}>
          {[
            ...
            <Link
              ...
              style={{ fontSize: 13, color: "#1565c0", textDecoration: "none", background: "#e3f2fd", borderRadius: 20, padding: "5px 14px" }}
            >
              ...
              <Grid item lg={3} sm={4} xs={6} key={region.slug}>
                <Link
                  ...
                  style={{
                    ...
                    padding: "0.75rem 1rem", fontSize: 13, color: "#1565c0",
                    textDecoration: "none", fontWeight: 600, background: "#fff",
                  }}
                >
                  💻 {isEn ? "Laptops in" : "Ноутбуки в"} {name}
